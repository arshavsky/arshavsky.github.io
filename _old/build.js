...
Beast.decl({
    Books: {
        requestApiV2: function (path, data) {
            ...
            var jsonUrl = 'https://5jnrq8s6.apicdn.sanity.io/v1/data/query/production?query=*[_type%20==%20%27book%27]%20|%20order(date%20desc)%20{%20title,%20author,%20date,%20highlight,%20remark%20}';
                ...
                success: function (data) {
                    ...
                    self.empty().append(
                        ...
                        Beast.node("BookList",{__context:this},"\n\n                                ",json.result.map(function(item, index) {
                                    ...
                                    //<year>{dateReadYear}</year>

                                    return (
                                        
                                        Beast.node("item",{"year":dateReadYear,"highlight":highlight},"\n                                            ",Beast.node("title",{"remark":remark},item.title),"\n                                            ",Beast.node("author",undefined,item.author),"\n                                        ")
...
Beast.decl({
    ...
    BookList__item: {
        expand: function () {
            ...
            var highlight = this.param('highlight')
            

            this.mod('highlight', highlight)

            this.append(
                Beast.node("year-title",{__context:this},year),
                this.get('title', 'author')
            )
        },

        domInit: function () {
            var liText = '';
            var liList = $('.booklist__year-title');
            var listForRemove = [];
    ...
    BookList__title: {
        expand: function () {
            ...
            if (this.param('remark')) {
                this.append(
                    Beast.node("remark",{__context:this},this.param('remark')),
                    Beast.node("title-wrap",{__context:this},this.text())
                )    
            } else {
                this.append(
                    Beast.node("title-wrap",{__context:this},this.text())
    ...
    'BookList__title-wrap': {tag: 'span'},

    'BookList__remark': {tag: 'span'},
